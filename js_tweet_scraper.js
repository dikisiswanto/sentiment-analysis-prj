...
function loop() {
	setTimeout(function () {
		...
		let element = document.querySelector('[data-testid=tweet]');
		for (let index = 0; index < element.length; index++) {
			try {
				...
				dict[element[index].innerText.split("\n")[1].concat("-" + time[1])] = {
					...
					"retweet": `${retweet[1]}`,
					"time": `${time[1]}`,
					"username": `${element[index].innerText.split("\n")[1]}`,
					"text": `${element[index].querySelector('[data-testid=tweetText]').innerText}`,
					"status": `${status[1]}`
				};
			} catch (error) {
				console.log(error);
